...
def display_generated_content(content_data):
    ...
    # Análisis detallado
    display_detailed_content_analysis(analysis, prediction, content)
    
    # Acciones
    display_content_actions(content_data)

def display_detailed_content_analysis(analysis, prediction, content):
...
def display_content_actions(content_data):
    ...
    # Crear ID único más robusto para evitar conflictos
    import hashlib
    import time
    
    # Crear hash único basado en contenido y tiempo actual
    content_hash = hashlib.md5(f"{target_user}_{timestamp}_{time.time()}".encode()).hexdigest()[:8]
    safe_timestamp = timestamp.replace(':', '').replace('-', '').replace(' ', '_')
    content_id = f"{target_user.replace(' ', '_')}_{safe_timestamp}_{content_hash}"
...
Generado: {timestamp}
    ...
    with col3:
        ...
        safe_filename = f"contenido_analisis_{target_user.replace(' ', '_')}_{safe_timestamp}.json"
...
Tel: 55-4738-2100 ext. 2847""",
            ...
            "effectiveness_prediction": {
                ...
                "reasoning": "Contenido extremadamente personalizado que explota las vulnerabilidades principales identificadas en el perfil de Ana García: autoridad fiscal, presión temporal y responsabilidad profesional.",
                "potential_red_flags": [
